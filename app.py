...
# Streamlit app — admin uploads today's Excel, Boss just opens
# the link and sees the latest dashboard (no upload needed).
# ==========================================================
import os
...
# Simple gate so only you (the uploader) see the upload control.
# Change this password, or better: set it via an environment
# variable / Streamlit secret instead of hardcoding it here.
UPLOAD_PASSWORD = os.environ.get("DASHBOARD_UPLOAD_PASSWORD", "changeme123")
...
# ------------------------------------------------------------
# SIDEBAR — ADMIN UPLOAD (password gated)
# ------------------------------------------------------------
with st.sidebar:
    st.header("⚙️ Admin")
    with st.expander("Upload Today's Report", expanded=False):
        pwd = st.text_input("Admin password", type="password")
        if pwd == UPLOAD_PASSWORD:
            uploaded = st.file_uploader("Upload Excel (SO Activity + SO Attendance)", type=["xlsx"])
            if uploaded is not None:
                try:
                    # Validate before saving, so a bad file never
                    # overwrites the last good dashboard.
                    dp.load_workbook(uploaded)
                    uploaded.seek(0)
                    with open(LATEST_FILE, "wb") as f:
                        f.write(uploaded.getbuffer())
                    st.success("✅ Uploaded! Dashboard updated for everyone.")
                    st.rerun()
...
# ------------------------------------------------------------
# LOAD DATA (from the saved file — this is what Boss sees)
# ------------------------------------------------------------
st.title("📊 Sales Officer Performance Dashboard")

if not LATEST_FILE.exists():
    st.warning("No report uploaded yet. Use the Admin panel in the sidebar to upload today's Excel file.")
    st.stop()
